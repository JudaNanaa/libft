...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/23 20:49:25 by madamou           #+#    #+#             */
/*   Updated: 2024/03/23 20:55:51 by madamou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	test_bzero(void)
{
	...
	st1 = malloc(sizeof(char) * 10);
	st2 = malloc(sizeof(char) * 15);
	ft_bzero(s1, 9);
	bzero(st1, 9);
...
static int	test_memcmp(void)
{
	...
	reussi = 0;
	if (ft_memcmp("_pp\0mm2", "_pp\0mm2", 5) == memcmp("_pp\0mm2", "_pp\0mm2",
			5))
		reussi++;
	else
		txt_error_info("ft_memcmp(\"_pp\\0mm2\", \"_pp\\0mm2\",
			5) == memcmp(\"_pp\\0mm2\", \"_pp\\0mm2\", 5)");
	if (ft_memcmp("\0\0\0", "\0\0\0", 3) == memcmp("\0\0\0", "\0\0\0", 3))
		reussi++;
	else
		txt_error_info("ft_memcmp(\"\\0\\0\\0\", \"\\0\\0\\0\",
			3) == memcmp(\"\\0\\0\\0\", \"\\0\\0\\0\", 3)");
	if (ft_memcmp("", "", 0) == memcmp("", "", 0))
		reussi++;
	else
		txt_error_info("ft_memcmp(\"\", \"\", 0) == memcmp(\"\", \"\", 0)");
	if (ft_memcmp("testosterogne.", "testosterone.",
			5) == memcmp("testosterogne.", "testosterone.", 5))
		reussi++;
	else
		txt_error_info("ft_memcmp(\"testosterogne.\", \"testosterone.\",
			5) == memcmp(\"testosterogne.\", \"testosterone.\", 5)");
	return (reussi);
}
...
static int	test_strdup(void)
{
	...
	if (!strcmp(ft_strdup(".egrgrhthtjdv\0"), strdup(".egrgrhthtjdv\0")))
		...
		txt_error_info("!ft_strcmp(ft_strdup(\".egrgrhthtjdv\\0\"),
			strdup(\".egrgrhthtjdv\\0\"))");
	return (reussi);
}
...
static int	test_strcpy(void)
{
	...
	reussi = 0;
	dst1 = malloc(sizeof(char) * 20);
	dst2 = malloc(sizeof(char) * 20);
	if (!ft_strcmp(ft_strcpy(dst1, "lololo"), strcpy(dst2, "lololo")))
		reussi++;
	else
		txt_error_info("!ft_strcmp(ft_strcpy(dst1, \"lololo\"), strcpy(dst2,
			\"lololo\"))");
	if (!ft_strcmp(ft_strcpy(dst1, "."), strcpy(dst2, ".")))
		...
		txt_error_info("!ft_strcmp(ft_strcpy(dst1, \".\"), strcpy(dst2,
			\".\"))");
	if (!ft_strcmp(ft_strcpy(dst1, ".egrgrhthtjdv\0"), strcpy(dst2,
				".egrgrhthtjdv\0")))
		reussi++;
	else
		txt_error_info("!ft_strcmp(ft_strcpy(dst1, \".egrgrhthtjdv\\0\"),
			strcpy(dst2, \".egrgrhthtjdv\\0\"))");
	return (reussi);
}
...
static int	test_strncpy(void)
{
	...
	if (!ft_strcmp(ft_strncpy(dst1, "lololo", 3), strncpy(dst2, "lololo", 3)))
		...
		txt_error_info("!ft_strcmp(ft_strncpy(dst1, \"lololo\", 3),
			strncpy(dst2, \"lololo\", 3))");
	if (!ft_strcmp(ft_strncpy(dst1, ".", 0), strncpy(dst2, ".", 0)))
		reussi++;
	else
		txt_error_info("!ft_strcmp(ft_strncpy(dst1, \".\", 0), strncpy(dst2,
			\".\", 0))");
	if (!ft_strcmp(ft_strncpy(dst1, ".egrgrhthtjdv\0", 9), strncpy(dst2,
				".egrgrhthtjdv\0", 9)))
		reussi++;
	else
		txt_error_info("!ft_strcmp(ft_strncpy(dst1, \".egrgrhthtjdv\\0\", 9),
			strncpy(dst2, \".egrgrhthtjdv\\0\", 9))");
	return (reussi);
}
...
static int	test_strsplit(void)
{
	...
	tmp = ft_split("...ee.test.1.oooo..", '.');
	if (!strcmp(tmp[0], "ee") && !strcmp(tmp[1], "test") && !strcmp(tmp[2], "1")
		&& !strcmp(tmp[3], "oooo") && !tmp[4])
		reussi++;
	else
	...
	tmp = ft_split("***********Bonjour************comment*******tu********vas*b",
			'*');
	if (!strcmp(tmp[0], "Bonjour") && !strcmp(tmp[1], "comment")
		&& !strcmp(tmp[2], "tu") && !strcmp(tmp[3], "vas") && !strcmp(tmp[4],
			"b") && !tmp[5])
		reussi++;
	else
	...
	if (!strcmp(tmp[0], "Bonjour") && !strcmp(tmp[1], "comment")
		...
		txt_error_info("ft_strsplit(\"***********Bonjour************comment*******tu********vas*b*\",
			'*');");
	tmp = ft_split("***********Bonjour************comment*******tu********vas*b*k",
			'*');
	if (!strcmp(tmp[0], "Bonjour") && !strcmp(tmp[1], "comment")
		&& !strcmp(tmp[2], "tu") && !strcmp(tmp[3], "vas") && !strcmp(tmp[4],
			"b") && !strcmp(tmp[5], "k") && !tmp[6])
		reussi++;
	else
