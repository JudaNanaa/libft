...
/*   Updated: 2024/03/30 20:01:12 by madamou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_ft_split(char *tab[][2], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		res = ft_split(tab[i][0], *tab[i][1]);
		wc = count_words(tab[i][0], tab[i][1]);
...
void	test_ft_strjoin(char *strs[][2], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		res_strjoin = ft_strjoin(strs[i][0], strs[i][1]);
		printf("%s \n", res_strjoin);
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
void	test_ft_striteri(char strs[][100], int size, void (*f)(unsigned int,
			char *))
{
...
void	test_ft_putchar_fd(char c)
{
	...
	fd = open("Test_ft_putchar_fd.txt", O_RDWR | O_CREAT, S_IWUSR | S_IRUSR);
	ft_putchar_fd(c, fd);
	close(fd);
...
void	test_ft_putstr_fd(char *s)
{
	...
	fd = open("Test_ft_putstr_fd.txt", O_RDWR | O_CREAT, S_IWUSR | S_IRUSR);
	ft_putstr_fd(s, fd);
	close(fd);
...
void	test_ft_putnbr_fd(void)
{
	...
	fd = open("Test_ft_putnbr_fd_INT_MAX.txt", O_RDWR | O_CREAT,
			S_IWUSR | S_IRUSR);
	ft_putnbr_fd(INT_MAX, fd);
	close(fd);
	printf("Adding : 2147483647 to test \n");
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	fd1 = open("Test_ft_putnbr_fd_INT_MIN.txt", O_RDWR | O_CREAT,
			S_IWUSR | S_IRUSR);
	ft_putnbr_fd(INT_MIN, fd1);
	close(fd1);
	printf("Adding : -2147483648 to test \n");
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	fd3 = open("Test_ft_putnbr_fd_minus_1.txt", O_RDWR | O_CREAT,
			S_IWUSR | S_IRUSR);
	ft_putnbr_fd(-1, fd3);
	close(fd3);
...
void	test_ft_putendl_fd(char *s)
{
	...
	fd = open("Test_ft_putendl_fd.txt", O_RDWR | O_CREAT, S_IWUSR | S_IRUSR);
	ft_putendl_fd(s, fd);
	close(fd);
	printf("Adding : '%s' to test \n", s);
	printf("------------Testing ft_putendl_fd-------------\n\n\n");
...
void	test_ft_strncmp(int tab[], char *strs[][2], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		printf("   strncmp('%s', '%s', %d) = %d\n", strs[i][0], strs[i][1],
			tab[i], strncmp(strs[i][0], strs[i][1], tab[i]));
		printf("ft_strncmp('%s', '%s', %d) = %d\n", strs[i][0], strs[i][1],
			tab[i], ft_strncmp(strs[i][0], strs[i][1], tab[i]));
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		}
...
void	test_ft_strnstr(int tab[], char *strs[][2], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		printf("ft_strnstr('%s', '%s', %d) = %s\n", strs[i][0], strs[i][1],
			tab[i], ft_strnstr(strs[i][0], strs[i][1], tab[i]));
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		}
...
void	test_ft_strchr(int tab[], char *strs[], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		printf("   strchr('%s', %d) = %p \n", strs[i], tab[i], strchr(strs[i],
				tab[i]));
		printf("ft_strchr('%s', %d) = %p \n", strs[i], tab[i],
			ft_strchr(strs[i], tab[i]));
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		}
...
void	test_ft_strrchr(int tab[], char *strs[], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		printf("   strrch('%s', %d) = %p \n", strs[i], tab[i], strrchr(strs[i],
				tab[i]));
		printf("ft_strrchr('%s', %d) = %p \n", strs[i], tab[i],
			ft_strrchr(strs[i], tab[i]));
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		}
...
void	test_ft_memchr(void)
{
	...
	test1 = "XXAABB";
	printf("   memchr('%s', %d) = %p \n", test1, (int)'A', memchr(test1,
			(int)'A', 6));
	printf("ft_memchr('%s', %d) = %p \n", test1, (int)'A', ft_memchr(test1,
			(int)'A', 6));
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	test2 = "";
	printf("   memchr('%s', %d) = %p \n", test2, (int)'A', memchr(test2,
			(int)'A', 0));
	printf("ft_memchr('%s', %d) = %p \n", test2, (int)'A', ft_memchr(test2,
			(int)'A', 0));
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	test3 = "1231234";
	printf("   memchr('%s', %d) = %p \n", test3, (int)'4', memchr(test3,
			(int)'A', 7));
	printf("ft_memchr('%s', %d) = %p \n", test3, (int)'4', ft_memchr(test3,
			(int)'A', 7));
	printf("------------Testing ft_memchr-------------\n\n\n");
}
...
void	test_ft_memcmp(int tab[], char *strs[][2], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		printf("   memcmp('%s', '%s', %d) = %d\n", strs[i][0], strs[i][1],
			tab[i], memcmp(strs[i][0], strs[i][1], tab[i]));
		printf("ft_memcmp('%s', '%s', %d) = %d\n", strs[i][0], strs[i][1],
			tab[i], ft_memcmp(strs[i][0], strs[i][1], tab[i]));
		if (i != (size - 1))
		{
			printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
		}
...
void	test_ft_calloc(void)
{
	...
	tab1 = calloc(10, sizeof(int));
	printf("   calloc(10, %ld) = '%s' \n", sizeof(int), tab1);
	printf("ft_calloc(10, %ld) = '%s' \n", sizeof(int), tab);
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	tab2 = ft_calloc(INT_MAX, sizeof(int));
	tab3 = calloc(INT_MAX, sizeof(int));
...
void	test_ft_lstadd_front(void)
{
	...
	printf("ft_lstnew(%s) = lst \n", test);
	printf("lst->content = %s\n", (char *)lst->content);
	lst2 = ft_lstnew("this is the new item !");
...
void	test_ft_lstdelone(void)
{
	...
	printf("list is : '%s' -> '%s' -> '%s' \n", (char *)lst->content,
		(char *)(lst->next)->content, (char *)((lst->next)->next));
	printf("------------Testing ft_lstdelone-------------\n\n\n");
}
...
t_list	*ft_lstmap(t_list *lst, void *(*f)(void *),
			void (*del)(void *)) __attribute__((weak));
...
void	test_ft_lstmap(void)
{
	...
	l = ft_lstmap(lst, func_test_lstmap, del_content);
	printf("the list contents should be 'HELLO' -> 'WORLD' -> '!' -> '(null)'\n");
	printf("list is : '%s' -> '%s' -> '%s' -> '%s' \n", (char *)l->content,
		(char *)(l->next)->content, (char *)((l->next)->next)->content,
		(char *)(ft_lstlast(l)->next));
	printf("------------Testing ft_lstmap-------------\n\n\n");
}
...
int	main(void)
{
	...
	char	*items_ft_atoi[] = {"123bn", "p123", "000122-+23\t23", "\t-123",
			"-\t123", "\t\t123", "-123zy"};
	...
	char	*items_ft_split[7][2] = {{"", "a"}, {"hello world !", " "},
			{"hello[world[!", "["}, {"Hello World !", ""}, {NULL, "a"},
			{"Hello World", ""}, {NULL, "A"}};
	char	*items_ft_strjoin[][2] = {{"Hello", NULL}, {NULL, "world"},
			{"Hello", "World"}, {"Hello", ""}, {"", "world"}, {"", ""}};
	char	*items_ft_substr1[] = {"Hello World !", "XXXXXCut from Here !", "",
			"XXXXX"};
	int		items_ft_substr2[] = {5, 5, 0, 4};
	char	*items_ft_strmapi[] = {"Hello World !", "Ici c'est Paris !", "",
			"Miaou"};
	char	items_ft_striteri[][100] = {"Hello World !", "Ici c'est Paris !",
			"", "Miaou"};
	char	*items_ft_strlcat[] = {"Add me !", "", "Easy !"};
	char	*items_ft_strlcpy[] = {"Add me !", "", "Easy !"};
	int		items_ft_strncmp1[] = {3, 2, 1, 2, 0, 5, 0};
	char	*items_ft_strncmp2[][2] = {{"ABC", "ABD"}, {"", ""}, {"hhhh",
			"hhhh"}, {"..", "z"}, {"z", ".."}, {"abc", "abc"}, {"", ""}};
	int		items_ft_strnstr1[] = {3, 3, 5, 4, 3};
	char	*items_ft_strnstr2[][2] = {{"abc", "d"}, {"abc", "b"}, {"dddda",
			"a"}, {"", "abc"}, {"acd", "acd"}};
	int		items_ft_strchr1[] = {(int)'o', (int)'!', (int)'x', (int)'a', 0};
	char	*items_ft_strchr2[] = {"Hello Hello", "Nothing here ! ! !",
			"XFind the X", "", "Heeeey there !"};
	int		items_ft_strrchr1[] = {(int)'x', 0, (int)'o', (int)'!', (int)'X',
				(int)'a', 0};
	char	*items_ft_strrchr2[] = {"", "", "Hello Hello", "Nothing here ! ! !",
			"XFind the X", "", "Heeeey there !"};
	char	*items_ft_strdup[] = {"Hello Hello", "Nothing here ! ! !",
			"XFind the X", "", "Heeeey there !"};
	int		items_ft_memcmp1[] = {3, 2, 1, 2, 0, 5, 0};
	char	*items_ft_memcmp2[][2] = {{"ABC", "ABD"}, {"", ""}, {"hhhh",
			"hhhh"}, {"..", "z"}, {"z", ".."}, {"abc", "abc"}, {"", ""}};
	...
	char *items_ft_strtrim[][2] = {{"Hello", "Ho"},
									{"XXXONLY_PASTE_THIS   ", "X "},
									{
										"",
										"ABC",
									},
	...
	test_ft_strrchr(items_ft_strrchr1, items_ft_strrchr2, 6);
	test_ft_memchr();
